...
def process_files(folder_root):
    df1 = pd.read_csv(str(folder_root) + "/train.csv")
    df2 = pd.read_csv(str(folder_root) + "/valid.csv")
    df3 = pd.read_csv(str(folder_root) + "/test.csv")
    ...
    df[["ltable_id","rtable_id"]].to_csv(str(folder_root) + "/matches.csv", header=True, index=False)
