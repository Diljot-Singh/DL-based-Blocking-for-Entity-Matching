...
#This function is used by SBERT
#It takes a list of tuple strings and outputs 2 pairs (T,T') and (T, L) where the first tuple is a match and the second is a non-match
def triplet_generate_synthetic_training_data(list_of_tuples, synth_tuples_per_tuple=5, 
        max_perturbation=0.4):
    
    num_tuples = len(list_of_tuples)
    total_number_of_elems = num_tuples * synth_tuples_per_tuple

    #We create three lists containing T, T' and L respectively
    original_tuple_list = [None for _ in range(total_number_of_elems)]
    modified_original_tuple_list = [None for _ in range(total_number_of_elems)]
    mismatch_tuple_list = [None for _ in range(total_number_of_elems)]
    ...
    for index in range(len(list_of_tuples)):
        ...
        training_data_index = index * synth_tuples_per_tuple

        #Create a list of perturbed tuple and a mismatch tuple
        for temp_index in range(synth_tuples_per_tuple):
            tokenized_tuple_copy = tokenizer(list_of_tuples[index])
            ...
            original_tuple_list[training_data_index] = list_of_tuples[index]
            modified_original_tuple_list[training_data_index] = ' '.join(tokenized_tuple_copy)
            mismatch_tuple_list[training_data_index] = random.choice(list_of_tuples)

            training_data_index += 1

    return original_tuple_list, modified_original_tuple_list , mismatch_tuple_list
...
class CTTCosineTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE),
    ...
    def preprocess(self, list_of_tuples):
        print("Training CTT Cosine model")
        self.sif_embedding_model.preprocess(list_of_tuples)
        ...
        trainer = dl_models.CTTCosineModelTrainer (self.input_dimension, self.hidden_dimensions)
...
class TripletTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE),
            synth_tuples_per_tuple=5, pos_to_neg_ratio=1, max_perturbation=0.4):
        super().__init__()
        self.input_dimension = EMB_DIMENSION_SIZE
    ...
    # this could be used to compute frequencies, train a DL model etc
    def preprocess(self, list_of_tuples):
        print("Training Triplet Loss SBERT model")

        original_l, modified_l, mismatch_l = triplet_generate_synthetic_training_data(list_of_tuples, 
            self.synth_tuples_per_tuple, self.max_perturbation)

        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        model = SentenceTransformer('distilbert-base-nli-mean-tokens', device = device)

        train_examples = [InputExample(texts=[original_l[i], modified_l[i], mismatch_l[i]]) for i in range(len(original_l))]
    ...
    # each tuple embedding is 1D numpy ndarray
    def get_tuple_embedding(self, list_of_tuples):
        embedding_matrix = self.model.encode(list_of_tuples)
        return embedding_matrix
...
class HybridTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE,2*AE_EMB_DIMENSION_SIZE),
            synth_tuples_per_tuple=5, pos_to_neg_ratio=1, max_perturbation=0.4):
        super().__init__()
        self.input_dimension = EMB_DIMENSION_SIZE
        self.hidden_dimensions = hidden_dimensions
        ...
        self.autoencoder_embedding_model = AutoEncoderTupleEmbedding( hidden_dimensions = self.hidden_dimensions)
        

    #This function is used as a preprocessing step 
    # this could be used to compute frequencies, train a DL model etc
    def preprocess(self, list_of_tuples):
        print("Training CTT model in Hybrid")
